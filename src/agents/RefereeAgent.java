    ...
    public void action() {
        if (this.game.isGameOver()) {
            ...
            try {
                ...
                // Esperar las respuestas de los jugadores
                int expectedResponses = 2;
        ...
        // Llamar al agente que debe jugar
            String currentPlayer = this.getPlayerName();
            ACLMessage message = new ACLMessage(ACLMessage.INFORM);
            message.addReceiver(new AID(currentPlayer, AID.ISLOCALNAME));
            try {
                message.setContentObject(State.IN_GAME); // Envía el objeto `TicTacToe` en lugar de `State.IN_GAME`
                myAgent.send(message);
            } catch (IOException e) {
                e.printStackTrace();
            }
